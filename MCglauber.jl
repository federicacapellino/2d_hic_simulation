...
"""
function nhard_profile(x,y,ncoll;rmax=6,ncoll_norm = 2*0.4087/(70.)/0.4)
       # ncoll_norm*exp(- (x^2 + y^2)/(2*(rmax/2)^2))*ncoll/(2*pi*(rmax/2)^2) #2d-gaussian with σ=rmax/2 
        ncoll_norm*ncoll*exp(-(x^2 + y^2)/(2*(rmax/2)^2))/(2*pi*(rmax/2)^2)
end
...
function charm_number_hard(ncoll;xmax=20.,ncoll_norm = 2*0.4087/(70.)/0.4)
    ...
    function f(u,p) 
        ncoll = p
        nhard_profile(u[1],u[2],ncoll,ncoll_norm=ncoll_norm)*0.4
    end
    p = (ncoll)
